<?
namespace Centurion\Core\Permission;

class Manager
{

    private $temp = 'core/var/PermissionBundles.php';
                    ...
                    {$files[] = $dir.'/'.$item;
                      $conf = require ($dir.'/'.$item);
                        ...
                        foreach ($conf['var'] as $num => $var) {
                          $this -> addPermission ($conf['path'] , $conf['var']);
                        }
    ...
    public function addPermission ($path = '', $arr) {
      $this -> addPath ($path);
      $cache = require($this -> temp);
    ...
    public function getPermission ( $path , $var )
      {
        $cache = require($this -> temp);
        return $cache[$path]['var'][$var]['value'];
      }

    public function updatePermission ( $path , $var , $value)
      {
        $cache = require($this -> temp);
